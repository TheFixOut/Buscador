...
public class BuscadorServiceImpl implements BuscadorService {
	...
	public List<Pagina> buscar(String clave){
		//return items.stream().filter(p->Arrays.stream(p.getSeo()).anyMatch(s->s.equals(clave))).collect(Collectors.toList());
		return items.stream().filter(p->Arrays.asList(p.getSeo()).contains(clave)).collect(Collectors.toList());
	}
}
